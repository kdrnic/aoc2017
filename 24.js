...
if(!fs.existsSync("./24.json")){
	console.log("Bridges not found, generating...");
	...
	fs.writeFileSync("24.json", JSON.stringify(bridges));
}
else {
	bridges = JSON.parse(fs.readFileSync("24.json").toString());
}
console.log(bridges.reduce((acc, e) => acc > e.w ? acc : e.w, 0));
longestL = bridges.reduce((acc, e) => acc > e.p.length ? acc : e.p.length, 0);
console.log(bridges.filter(b => b.p.length == longestL).reduce((acc, e) => acc > e.w ? acc : e.w, 0));
/*
